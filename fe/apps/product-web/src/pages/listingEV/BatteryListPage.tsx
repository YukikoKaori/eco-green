...
import { fetchBatteriesPaged, type BatteryItem } from "@/api/battery";
import { BatteryFilterBar, type BatteryFilterState } from "@/components/filters/BatteryFilterBar";
...
export default function BatteryListPage() {
  ...
  useEffect(() => { setPage0(0); }, [filters]);
  useEffect(() => {
    const ac = new AbortController();
    (async () => {
      try {
        setLoading(true);
        const res = await fetchBatteriesPaged({
          filters: {
            city: filters.city,
            brand: filters.brand,
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-3 md:px-6 py-4">
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="text-center text-slate-600 py-8">Không tìm thấy pin phù hợp.</div>
      ) : (
        <div className="space-y-3">
          {items.map((p) => {
            const firstImg =
            ...
            return (
              <div key={p.id} className="bg-white rounded-lg shadow-sm">
                <RowListing
                  id={p.id}
                  title={p.title || p.brandName || "Pin xe điện"}
                  price={fmtVND(p.price)}
                  subtitleLeft={p.conditionType === "NEW" ? "Pin mới" : "Pin đã dùng"}
        ...
        {pages.map((p) => (
          <button
            ...
            className={`px-3 py-1.5 border rounded-md text-sm ${
              p - 1 === page0 ? "!bg-[#00BFAE] !text-white !text-sm !border-emerald-600" : "hover:bg-slate-50"
            }`}
