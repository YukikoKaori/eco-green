...
import { fetchProvinces, type Province } from "@/api/vehicle";
import { fetchBatteryBrands, type BatteryBrand } from "@/api/battery";

/* ======================== Types ======================== */
export type BatteryFilterState = {
    city: string;
    brand: string;       // id hoặc code
    minPrice?: number;
    maxPrice?: number;
    yearFrom?: number;
    yearTo?: number;
...
export function BatteryFilterBar({
    value, onChange, onClearAll,
}: {
    value: BatteryFilterState;
    onChange: (v: BatteryFilterState) => void;
    onClearAll?: () => void;
}) {
    const [open, setOpen] = useState<null | "city" | "brand" | "price" | "year">(null);
    const [provinces, setProvinces] = useState<Province[]>([]);
    const [brands, setBrands] = useState<BatteryBrand[]>([]);

    useEffect(() => { fetchProvinces().then(setProvinces).catch(() => setProvinces([])); }, []);
    useEffect(() => { fetchBatteryBrands().then(setBrands).catch(() => setBrands([])); }, []);
